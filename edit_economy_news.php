<?php
	...
	//{
		...
		$NewsID = $_GET['id'];
		$sql   = "SELECT * FROM economy_news WHERE news_id = '$NewsID'";
		$res   = $conn->query($sql);
		if ($res->num_rows > 0) {
			$row = $res->fetch_assoc();
		}	
...
// Create connection
	...
	if (isset($_POST['submit'])) {
		...
		$query = "UPDATE economy_news SET news_title='$news_title', news_image='$news_image', news_short_description='$news_short_description', news_full_content='$news_full_content', news_author='$news_author', news_published_on='$news_published_on' WHERE news_id=$news_id";
